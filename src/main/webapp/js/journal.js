...
// 手账下：创建一条素材
function createJoMaterialOne(id,mid,imgurl,name,price,number){
  var joCommentBox = document.getElementById("joCommentBox");
    ...
    $(joMaterial).attr("mid",""+id);
      ...
      var mdel = createEle("div","buyBtn");
      mdel.innerHTML="删除";
      $(mdel).click(function(){
    	 meDel(id); 
      })
    addChilds(joMaterial,meImg,meName,meStock,mePrice,mdel);
  addChilds(joCommentBox, joMaterial);
}
// 删除请求
function meDel(id){
  var path=$("#basePath").val()+"/material/del";
  var d={
      id:id
  }
  $.ajax({
     ...
     contentType: 'application/json',
      ...
      success:function (data) {
        $.each(data, function(idx, obj) {
          ...
          createJoMaterialOne(id,mid,imgurl,name,price,number)
            // 添加的，当条数达到5的时候，停止生成html，并添加分页按钮
            if(idx==obj.page.pageNumber-1){
              var currentPage=obj.page.currentPage;
              var totalPage=obj.page.totalPage;
...
function subComment(tid){
  ...
  var searchPath = $("#basePath").val()+"/comment/add";
  var d={
      content:comment,
      tid:tid
  }
